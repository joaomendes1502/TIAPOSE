...
for (i in 4:7) {
  ...
  # Evaluate Forecasts
  reshow(colnames(TS)[i], Y, PredRF, "MAE")
  cat("MAE:",mmetric(Y,PredRF,metric="MAE"),"\n")
  cat("NMAE:",mmetric(Y,PredRF,metric="NMAE",val=YR),"\n")
  cat("RMSE:",mmetric(Y,PredRF,metric="RMSE"),"\n")
  cat("RRSE:",mmetric(Y,PredRF,metric="RRSE"),"\n")
  cat("R2:",mmetric(Y,PredRF,metric="R22"),"\n") # press R measure
  mpause()
}
