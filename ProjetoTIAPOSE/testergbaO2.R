...
eval_rgba2 <- function(s, previsoes_vendas) {
  ...
  return(list(profit = month_profit, effort = month_effort))
}
...
rgbao2_optimizar <- function(previsoes_vendas) {
  lower <- rep(0, 28)
  upper <- calcular_upper(previsoes_vendas)
  
  # Definir a função de avaliação para o algoritmo genético
  eval_fn_ga <- function(s) {
    res <- eval_rgba2(s, previsoes_vendas)
    # Combinar os objetivos em uma função escalarizada
    w1 <- 0.5
    w2 <- 0.5
    K <- 500000  # Pode ajustar esses parâmetros conforme necessário
    objective_value <- -w1 * res$profit + w2 * res$effort * K  # Maximizar lucro e minimizar esforço
  ...
  melhor_resultado <- eval_rgba2(melhor_solucao, previsoes_vendas)
  
  return(list(
    solucao = melhor_solucao, 
    lucro = abs(melhor_resultado$profit),
    effort = melhor_resultado$effort
  ))
}