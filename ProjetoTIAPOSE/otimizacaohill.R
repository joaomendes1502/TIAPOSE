source("hill.R")  # Importar as funções necessárias para Hill Climbing

rchange1 <- function(par, lower, upper) {
  hchange(par, lower = lower, upper = upper, rnorm, mean = 1, sd = 0.2, round = TRUE)  # Pequenas mudanças
}
...
print("Matriz com os valores reais:")
...
print("Matriz com os valores previstos:")
print(previsoes_vendas)


# Função de avaliação conforme fornecida
eval <- function(s) {
  s <- round(s)  # Garante que os valores são inteiros
  ...
  for (s in 1:4) {
    for (d in 1:4) {
      if (s == 1) {
        ...
        stock[s - 1, d] + product_orders[s, d] - sales[s, d]
      }
    }
  }
...
# Definir limites inferior e superior para Hill Climbing
...
control <- list(maxit = 5000, REPORT = 100)  # Número máximo de iterações e frequência de relatórios

# Aplicar Hill Climbing
resultado_hill_climbing <- hclimbing(
  par = runif(28, lower, upper),  # Solução inicial
  fn = eval,
  change = rchange1,
  lower = lower,
  upper = upper,
  control = control,
  type = "max"  # Maximizar o lucro
)

# Melhor solução encontrada pelo Hill Climbing
print("Melhor solução com Hill Climbing:")
print(round(resultado_hill_climbing$sol))  # Solução com 28 valores (funcionários e encomendas)

# Melhor lucro obtido com Hill Climbing
print(paste("Melhor lucro obtido:", resultado_hill_climbing$eval))
