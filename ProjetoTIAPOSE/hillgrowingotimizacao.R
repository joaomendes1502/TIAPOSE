...
library(rminer)
source("hill.R")  # Função para Hill Climbing

# Função para gerar previsões usando Growing Window
generate_forecasts <- function(dep_data, Test, S, Runs, W2, D) {
  ...
  for (b in 1:Runs) {
    ...
    forecasts[b, ] <- round(previsoes)  # Arredondar previsões
  }
  
  return(forecasts)
}

# Definir a função de avaliação para Hill Climbing
eval <- function(s, vendas_previstas) {
  ...
  # Custos por tipo de trabalhador
  custo_junior <- 6000
  custo_normal <- 8000
  custo_senior <- 9750
...
# Função para pequenas mudanças em Hill Climbing
rchange1 <- function(par, lower, upper) {
  hchange(par, lower = lower, upper = upper, rnorm, mean = 0, sd = 0.25, round = TRUE)  # Pequenas mudanças
}

# Controle para Hill Climbing
control <- list(maxit = 1000, REPORT = 100)

# Calcular previsões com Growing Window
TS <- read.table("walmart.csv", sep = ",", header = TRUE)
vendas_previstas <- matrix(0, nrow = 4, ncol = 4)  # Matriz para previsões
...
for (i in 4:7) {
  ...
  # Preencher a matriz `vendas_previstas` com as previsões
  vendas_previstas[, i - 3] <- forecasts[nrow(forecasts), ]
}

# Aplicar Hill Climbing com as previsões do Growing Window
lucros <- vector("numeric", Runs)
solucoes <- list()

for (b in 1:Runs) {
  resultado <- hclimbing(
    par = runif(28, lower, upper),  # Solução inicial aleatória
    fn = function(s) eval(s, vendas_previstas),
    change = rchange1,
    lower = lower,
    upper = upper,
    control = control,
