...
eval_de <- function(s, previsoes_vendas) {
  ...
  w1 <- 0.5
  w2 <- 0.5
  K <- 500000  # Ajuste conforme necessário
  objective_value <- -w1 * month_profit + w2 * month_effort * K  # Maximizar lucro e minimizar esforço
  
  return(list(objective = objective_value, profit = -month_profit, effort = month_effort))
}
...
# Função para otimização com Differential Evolution
deo2_optimizar <- function(previsoes_vendas) {
  lower <- rep(0, 28)
  upper <- calcular_upper(previsoes_vendas)
  
  # Executar otimização Differential Evolution
  resultado <- DEoptim(fn = function(s) eval_de(s, previsoes_vendas)$objective, lower = lower, upper = upper, control = DEoptim.control(itermax = N, trace = 0))
  
  melhor_solucao <- round(resultado$optim$bestmem)
  
  final_results <- eval_de(melhor_solucao, previsoes_vendas)
  
  return(list(
    solucao = melhor_solucao,
    lucro = final_results$profit,
    esforco = final_results$effort
  ))
}
