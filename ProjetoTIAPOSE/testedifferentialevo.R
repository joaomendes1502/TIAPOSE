...
eval <- function(s, previsoes_vendas) {
  ...
  # Retornar o lucro (F1)
  return(-month_profit)  # Negativo para DE otimizar corretamente (minimização)
}
...
# Parâmetros da Differential Evolution
N <- 30

# Função para otimização com Differential Evolution
de_optimizar <- function(previsoes_vendas) {
  lower <- rep(0, 28)
  upper <- calcular_upper(previsoes_vendas)
  
  # Executar otimização Differential Evolution
  resultado <- DEoptim(fn = function(s) eval(s, previsoes_vendas), lower = lower, upper = upper, control = DEoptim.control(itermax = N, trace=0))
  
  melhor_solucao <- round(resultado$optim$bestmem)  # Arredondar para garantir valores inteiros
  melhor_lucro <- -resultado$optim$bestval  # Reverter sinal para obter o lucro positivo
