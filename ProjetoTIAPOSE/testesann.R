...
# Função Simulated Annealing ajustada para receber previsões
sann_optimizar <- function(previsoes_vendas) {
  ...
  resultado <- optim(
    ...
    fn = function(s) eval(s, previsoes_vendas),
  ...
  melhor_solucao <- round(resultado$par)  # Arredondar para garantir valores inteiros
  melhor_lucro <- -resultado$value  # Reverter sinal para obter lucro positivo
