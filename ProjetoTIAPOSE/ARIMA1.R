  ...
  reshow <- function(label = "", Y, Pred, metric = "MAE", PLOT = TRUE, val= NULL) {
  ...
  for (i in 4:7) { #vai fazer um loop desde a coluna 4 (dep1) até à coluna 7 (dep4)
    ...
    F2 <- forecast(AR, h = H) #o modelo ARIMA vai fazer previsões para H passos à frente. essas previsoes são guardadas na variavel F2
    Pred2 <- F2$mean[1:H] #pega nos primeiros valores de H e guarda na variavel Pred2
    
    reshow(colnames(TS)[i], Y, Pred2, "MAE")
    cat("MAE:",mmetric(Y,Pred2,metric="MAE"),"\n")
    cat("NMAE:",mmetric(Y,Pred2,metric="NMAE",val=YR),"\n")
    cat("RMSE:",mmetric(Y,Pred2,metric="RMSE"),"\n")
    cat("RRSE:",mmetric(Y,Pred2,metric="RRSE"),"\n")
    cat("R2:",mmetric(Y,Pred2,metric="R22"),"\n") # press R measure
    mpause()
  }
